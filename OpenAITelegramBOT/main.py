...
class DialogBot(object):
    ...
    async def start_command(self, update, context):
        ...
        tokens = int(float(self.get_value(chat_id, 'tokens'))) if int(float(self.get_value(chat_id, 'tokens'))) > 0 else 0
    ...
    async def handle_message(self, update, context):
        ...
        try:
            ...
            tokens = int(float(self.get_value(chat_id, 'tokens')))
    ...
    def gpt3_chat_model(self, chat_id, text, model):
        '''
        The `gpt3_chat_model` function is a method of a class that takes three parameters: `chat_id`, `text`, and `model`.
        The function retrieves previous messages from the chat history and limits the total number of tokens to be fed
        to the GPT-3 model.
        The function then sends the messages and the user's input to the OpenAI API and receives a response.
        The function captures the number of tokens used and updates the usage statistics for the model.
        Finally, the function adds the response to the chat history and returns a formatted string that includes
        the response and the usage statistics.
        :param chat_id: ID of the chat session.
        :param text: Text input from user.
        :param model: GPT-3 model selected for chat session.
        :return: String response generated by GPT-3 model.
        '''
        tokens_cnt = 0
        messages_temp = []
        conv_toks = int(s.MAX_MODEL_TOKENS[model] / 2)
    ...
    def get_used_tokens(self, chat_id, model, used_tokens):
        ...
        tokens = int(float(self.get_value(chat_id, 'tokens')))
        self.set_value(chat_id, 'tokens', int(tokens - used_tokens))
        return used_tokens
...
def get_markup(tokens):
    ...
    if tokens > s.MINIMUM_TOKENS['dalle']:
        items.append([item5])
